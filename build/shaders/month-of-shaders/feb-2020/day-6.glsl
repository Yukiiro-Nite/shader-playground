// map image brightness to a height
...
void main() {
    ...
    float brightness = luminosity(texture2D(u_tex0, vec2(pos.x * 0.1, fract(u_time * 0.05))));
    float brightness2 = luminosity(texture2D(u_tex0, vec2(pos.x * 0.1, fract((u_time * 0.05) + 0.02))));
    float brightness3 = luminosity(texture2D(u_tex0, vec2(pos.x * 0.1, fract((u_time * 0.05) + 0.04))));
    float brightness4 = luminosity(texture2D(u_tex0, vec2(pos.x * 0.1, fract((u_time * 0.05) + 0.06))));
