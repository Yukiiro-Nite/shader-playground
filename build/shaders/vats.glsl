...
void main() {
    ...
    vec2 ratio = vec2(1.0) / u_resolution.xy;
    vec4 imageColor = texture2D(u_tex0, pos);
    vec4 outlineColor = applyKernel(pos, ratio, u_tex0, outline);
    vec4 color = mix(imageColor, vec4(0.0, 1.0, 0.0, 1.0), outlineColor.r);
